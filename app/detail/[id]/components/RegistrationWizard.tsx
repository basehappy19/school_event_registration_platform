    ...
    <div className="bg-white sm:rounded-3xl sm:shadow-xl sm:border border-slate-100 overflow-hidden relative">
      ...
      <div id="registration-section" className="px-5 py-6 sm:p-8">
        ...
        <div className="space-y-8">
          {/* User Status Section */}
          {!session ? (
            <div className="bg-gradient-to-br from-indigo-50/90 via-white to-slate-50 border border-indigo-100/80 p-5 sm:p-6 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 shadow-xs">
              <div className="text-center sm:text-left w-full sm:w-auto">
                <h3 className="font-bold text-slate-800 text-base sm:text-lg mb-2">เข้าสู่ระบบเพื่อลงทะเบียน</h3>
                <div className="inline-flex items-center justify-center sm:justify-start gap-1.5 bg-rose-50 border border-rose-200/80 text-rose-700 font-bold px-3 py-1 rounded-full text-xs shadow-2xs">
                  <ShieldCheck className="w-3.5 h-3.5 text-rose-500 shrink-0" />
                  <span>เฉพาะอีเมลโรงเรียน (@phukhieo.ac.th)</span>
                </div>
                {isBeforeStart && (
                  <p className="text-emerald-600 text-xs mt-2.5 leading-relaxed">
                    💡 <span className="font-semibold">ทริค:</span> ล็อกอินรอก่อนเวลาเปิดได้เลย ปุ่มจะเปิดให้กดทันที!
                  </p>
                )}
              </div>
              <form action={signInWithGoogle.bind(null, project.id)} className="w-full sm:w-auto shrink-0 mt-1 sm:mt-0">
                <button
                  type="submit"
                  className="w-full sm:w-auto justify-center bg-white hover:bg-slate-50 text-slate-700 font-bold py-3 px-5 rounded-xl transition-all shadow-sm flex items-center border border-slate-200/80 active:scale-[0.98]"
                >
          ...
          <div className="mt-6 bg-slate-50 border border-slate-100 rounded-2xl p-5 relative">
            ...
            <div className="relative w-full mt-10 mb-12 z-10 pointer-events-none">
            ...
            {/* Note about waitlist / reserve */}
            <div className="mt-6 bg-amber-50/90 border border-amber-200/80 rounded-xl p-3.5 sm:p-4 flex items-start gap-3 shadow-xs">
              <AlertCircle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
              <div className="text-xs sm:text-sm text-amber-900 leading-relaxed">
                <span className="font-bold text-amber-950">หมายเหตุ:</span> หากจำนวนที่ลงทะเบียนเข้ามาเกินที่ตั้งไว้ ยังสามารถลงทะเบียนเข้ามาได้แต่จะเป็นสำรอง ทั้งนี้ขึ้นอยู่กับการพิจารณาของคณะครู
              </div>
            </div>
          </div>
        </div>
